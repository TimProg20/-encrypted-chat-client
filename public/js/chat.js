...
function initDialog(userId, userLogin, checkMessages) {
    ...
    let response = await encryptMessage({ userId: userId, value: messageInput.value });

    if (!response.success) {
      createErrorMessage(response.result);
    }
...
function insertUserWithTabToList(userBlock, tabBlock, user, position = 'beforeend') {
  ...
  document.getElementById(tabBlock).insertAdjacentHTML('beforeend',
    `<div id="userTab-${ user.id }" class="tab-pane fade" role="tabpanel">
      ${(checkMessages) ? `<button class="btn btn-primary d-flex my-3 mx-auto showMessagesButton" data-user-id="${ user.id }">See messages</button>` : ""}
...
socket.on('showMessages', function(data) {
  ...
  data.result.results.forEach(async function(item) {

    let response = await decryptMessage({ userId: secondUserId, value: item.value });

    if (!response.success) {
      createErrorMessage(response.result);
    }

    showMessage(secondUserId, item.senderLogin, response.result, 'afterbegin');
  });
...
socket.on('userJoined', function(data) {
  ...
  if (auth.id !== data.userId) {
    ...
    let userListItem = document.getElementById('oldUsersList').querySelector(`.userLink[data-user-id="${data.userId}"]`);
...
socket.on('createMessage', async function(data) {
  ...
  let response = await decryptMessage({ userId: data.userId, value: data.value });

  if (!response.success) {
    createErrorMessage(response.result);
  }

  showMessage(data.userId, data.login, response.result, 'beforeend');
